...
from competition.intake import validate_delivery
from competition.validation import CaseInputs, validate_case_inputs
...
def adapt_wuhan_delivery(source_root: str | Path, output_dir: str | Path, *, year: int = 2025) -> WuhanAdaptationResult:
    ...
    manifest = Path(__file__).resolve().parents[1] / "configs" / "wuhan_delivery_intake.yaml"
    ...
    (output / "adapter_run.log").write_text(f"adapter=competition.adapters.wuhan_delivery\nyear={year}\nrandom_seed=202611\nsource_root={source}\noutput_dir={output}\nsource_files_read_only=true\n", encoding="utf-8")
