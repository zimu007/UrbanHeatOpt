...
from competition.road_joint_v2.economic_package import PACKAGE_DIRECTORY, read_package, effective_parameters, write_gap_report, describe_energy_input
from competition.road_joint_v2.network import atomize_snapshot, write_network, RoadNetworkError
from competition.road_joint_v2.builder import build_season_case
from competition.road_joint_v2.tasks import create_plan, run_task, write_json
...
def prepare_delivery(delivery_root, osm_snapshot, output_root, run_id, *, profile='v0-full-season',desktop_report=None):
    ...
    try:
        ...
        network=atomize_snapshot(json.loads(Path(osm_snapshot).read_text(encoding='utf-8')),data.buildings,
            data.loads.groupby('building_id').heating_kW.sum().to_dict())
