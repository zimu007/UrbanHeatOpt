...
## 2026-09-07 — A/B接口接入V2教师冻结参数

- 更改内容：严格读取`0907_V2参数冻结补丁/scenario_parameter_manifest.csv`及其12个受控文件；默认正式经济情景使用扩容后的三档管网费用/热损和300万元站房费，调试管价只允许程序联调，1800万元站房费作为替换型压力情景。DN250/400/500物理容量仅作审计，执行容量只读`pipe_capacity_limits.csv`。
- 目的：把老师确认的管容量、TES上限和站房费用从说明文档转为可哈希、可校验、可供模型消费的机器边界，消除A/参数组向B交接的最后外部参数缺口。
- 接口：CaseBundle记录所选V2情景及成本边界；B1映射扩容管价、管损和仅建站时计取的站房投资，B2消费规划容量/TES边界。就绪状态新增`program_feasibility_input_ready`、`economic_conclusion_input_ready`和`economic_result_reliable`，避免把输入就绪误报为结果可靠。
- 输出：标准结果的`storage_decisions.csv`增加TES实际能量容量、实际最大充/放功率、三类上限和是否触及上限；TES不抵扣20%峰值容量裕度。
- 验证：全量测试`681 passed / 3 skipped`；真实只读prepare检查459个输入文件，62栋×2160h=133920行；输入、参数、标准数据、A/B能力和经济结论输入均就绪，源文件前后SHA-256一致，外部缺失数据0项。`solver_executed=false`、`result_qualified=false`、`economic_result_reliable=false`，本节点未启动2160小时求解。
- 风险：五个候选站仍是研究用虚拟站点；虽然经济输入口径已冻结，只有B侧完成求解且独立QA复算通过后，才可称经济结果可靠。本机HighsPy 1.11.0低于仓库锁定的1.15.1，后续正式长求解环境须按锁定版本配置。本次不提交原始输入、运行结果或桌面副本，不推送远端。
...
# 2026-09-07 三层接线节点1：固化道路快照并统一网络产物

- 修改：将已验证OSM快照及来源、许可、SHA-256和规划策略登记到`baselines/spatial/guanggu_osm_20260828`；A侧`prepare`改为从显式基线确定性生成道路原子边、5个道路吸附候选站、每栋最多3个接入方案、GIS/CSV/预览和网络manifest。CaseBundle现包含网络产物哈希，不再把未吸附道路的虚拟点作为权威站址，也不从历史`runs`目录发现网络。
- 目的：使负荷、候选站、道路网络和0907容量边界使用同一组站点ID与空间哈希，为RoadCase Builder提供唯一可复现网络输入；该网络只用于源荷匹配和规划优化，不宣称施工可实施。
- 验证：网络/A接口专项`48 passed`；真实v0.3两次独立prepare均退出0，得到399节点、578原子边、5站、184接入方案，网络文件、站点和接入候选SHA-256逐项一致；62栋×2160小时标准输入、0907参数及461项源/基线文件哈希前后不变。
- 边界：本节点未构建RoadCase、未实例化HiGHS、未生成全季优化结果；紧凑求解范围仍限定为5个候选站的确定性道路树。

# 2026-09-07 三层接线节点2：建立CaseBundle到RoadCase唯一构建器

- 修改：新增`build_road_case`生产构建器、路径无关CaseBundle内容ID、RoadCase内容哈希及构建报告；`prepare`现在自动写出`road_case.json`、`road_case_build_report.json`和`road_case_hashes.json`。修正B1适配后的技术状态枚举，并允许老师确认的0907研究容量状态通过B2执行校验。
- 目的：确保62栋负荷、2160小时时序、网络、0907容量/TES边界和20260831经济参数被编译进同一个不可变RoadCase，消除“参数已读取但模型仍使用旧案例”的断链风险；不读取历史runs或旧RoadCase。
- 验证：接口专项56项通过；真实CaseBundle复核481个产物/来源文件，成功构建62栋、2160小时、133920条负荷、399节点、578边、5站、184接入方案的RoadCase。站房300万元、需量费42元/kW·月、LHV锅炉、三档规划容量和TES 793606.88 kWh/132267.81 kW均进入案例，22个高温小时按既定上界封顶，无低温越界；输入哈希不变。
- 边界：本节点尚未接通SolveRequest执行器，因此`solver_pipeline_ready/research_solve_ready=false`，未实例化HiGHS、未产生全季优化结果；数学目标和物理约束未修改。

# 2026-09-07 三层接线节点3：接通SolveRequest生产执行与结果证据

- 修改：新增哈希锁定的SolveRequest生产执行器与`run.py solve`入口；请求固定声明紧凑五树模型档案、五候选最短路树优化范围和禁止未供热。分布式求解一次，集中式/混合式逐站求解5棵确定性道路树，全部子任务合格后才合成全局下界、选择最优站点并生成ResultBundle。输出包含候选站比较、求解证据、选中方案、容量/接网/管网/逐时调度、成本/碳排及独立QA。
