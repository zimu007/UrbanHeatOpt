...
# 2026-09-08 GUI阶段代码归档与启动检查

- 修改：提交已完成的桌面GUI阶段代码、前后端接口、演示数据、结果导出、组件化界面和两份中文设计说明；顶层包改用惰性导出，避免仅启动界面时提前加载求解链。补充`run_gui.py`和`gui.bat`启动入口，并移除批处理文件中的个人绝对Python路径。
...
- 启动验收补充：首次`--pytest`实跑发现Windows批处理在括号块中提前展开`%*`，造成控制参数被重复传给pytest；改用标签分支后重新验证参数转发。该问题不涉及求解模型或环境版本。

# 2026-09-08 接通无TES的碳端点与Pareto膝点入口

- 修改：统一`run.py solve`增加`carbon-endpoints`和`pareto-knee`请求集；最低碳端点采用“先最小化物理碳排，再在认证碳下限内最小化成本”的词典序求解。每种模式在成本、碳端点间最多求解25%、50%、75%三个ε点，输出完整ResultBundle、Pareto表、分模式/合并前沿和归一化膝点。
- 目的：让已有碳表达式和ε约束真正从生产CaseBundle进入紧凑五树核心，而不是只存在于单元测试或历史任务脚本；端点重合时明确停止内部扫描，不重复计算或伪造膝点。
- 认证：ε约束下经HiGHS证明不可行的固定站点视为有效排除证据；任何未认证失败、gap超限或QA失败仍使整个点封闭失败。所有最终点均保留五树候选比较和完整物理决策。
- 边界：本节点只接无TES前沿；`full-study`在固定站址、接网及管型的TES配对实现前明确拒绝运行，不把自由重优化的TES结果冒充配对结论。

# 2026-09-08 接通TES固定结构配对与完整研究入口

- 修改：执行器升级为`solve_request_executor_1.2.0`；`full-study`先生成三模式无TES成本—碳排前沿，再对集中式和混合式的分模式膝点固定候选站、逐栋接网决策及每段管型，只重新优化设备容量、逐时调度和TES。少于3个不同非支配点时不伪造膝点，明确回退到成本端点作为配对参考；纯分布式因没有区域能源站TES标记为不适用。
- 目的：使TES公式、2160小时SOC及上下限真正进入生产SolveRequest链，同时保证有/无TES差异不是由换站、换接网方案或换管型造成。输出TES实际能容量、最大充放功率、上限触及状态、成本/碳排差值及固定结构QA。
- 验证：2栋×2小时真实Pyomo/HiGHS合成链完成三模式Pareto和集中式/混合式TES固定结构配对；执行器、Pareto、交接、TES及A集成专项`114 passed`，固定环境下最终全量回归`712 passed / 3 skipped`。
- 边界：TES ON配对只认证固定结构下的再调度结果；它不重新选择五个候选站或接网/管网结构。真实62栋×2160小时任务需另行完成全部求解、gap及独立QA，才能称为全季研究结果。

# 2026-09-08 接通TES固定结构配对与完整研究入口

- 修改：`full-study`先执行三模式无TES的ε-constraint前沿，再对集中式、混合式各自的模式膝点固定候选站、逐栋接网向量和逐边管型，仅重新优化设备容量、逐时调度及TES容量/充放/SOC；纯分布式明确记为无区域站TES、不适用。
- 目的：让TES真正进入生产求解，同时保证有/无TES对照没有借更换站址、管网或接网建筑获得不公平优势。当前前沿不足三个不同点时不伪造膝点，而明确使用成本端点作为配对参考。
- 输出与QA：新增固定结构哈希、固定结构文件和配对QA；报告TES实际能量/充放功率容量、逐时最大充放功率、是否触及0907冻结上限、成本与碳排差值，并复核管网及接入年化成本不变、独立QA通过。
- 边界：`full-study`仍属于紧凑五候选最短路树优化范围，不代表完整自由拓扑或施工级管网最优。真实62栋×2160小时结果只有在新固定环境下全部点和TES配对均通过求解gap及独立QA后才可称完成。

# 2026-09-08 消除整数容差放大的假管段残差

- 问题：首次真实`full-study`已完成集中式和分布式前沿，但混合式词典序碳端点中两个候选站被独立QA拒绝。HiGHS给出的建管二元值约为`5e-10～8e-10`，乘以大容量上限后形成约`2e-5 kW`的假容量/假流量；这是求解器整数容差残差，不是网络物理不可行。
- 修改：生产执行器在MIP取得有效界后检查全部二元变量；只有所有二元值距0/1不超过`1e-6`时，才将已认证离散方案精确固定到0/1并重解连续调度。原MIP的best bound和gap证据继续保留，固定设计重解只用于消除数值残差、导出和独立QA；超过容差则封闭失败。每个候选任务新增`integer_polish.json`，实际触发时另存`polish_solver.log`和求解证据。
- 目的：避免对全局物理QA放宽容差，同时消除极小整数残差经大容量系数放大造成的“未建管段有容量/流量”假阳性；不改变目标函数、离散设计、时间范围、候选站或数学可行域。
- 验证：新增容差残差清除与实质非整数拒绝测试；执行器专项`13 passed`，固定环境全量回归`714 passed / 3 skipped`，`git diff --check`通过。首次失败运行目录保留不覆盖，后续使用新RUN_ID重新prepare和执行全季研究。

# 2026-09-08 消除整数容差放大的假管段残差

- 问题：真实全季混合式最低碳词典序阶段中，HiGHS返回约`5e-10～8e-10`的二元变量整数容差残差；该残差乘以约4万kW的规划管段容量后，被放大成约`2e-5 kW`的假容量/假流量，导致严格独立QA把两个候选站误判为未建管段有容量。
- 修改：在MIP已通过gap认证后检查全部二元变量；只有每个变量均在`1e-6`内接近0/1且最大残差超过`1e-12`时，才把原离散方案精确固定到最近整数并重解连续LP。原MIP下界和gap证据完整保留，精修只用于消除数值尾差、导出决策及独立QA；若存在实质性分数变量则封闭失败。
- 目的：不放宽热平衡、管段容量或未建零流的`1e-6`物理QA门槛，也不改变网络、目标函数和离散方案，避免用降低验收标准掩盖求解器容差。
- 证据：每个候选任务新增`integer_polish.json`；触发时另存`polish_solver.log`和`polish_solver_evidence.json`，记录精修前后最大整数残差及被固定的二元变量数量。

# 2026-09-09 最终优化节点1：并行候选求解与TES语义验收

- 修改：求解配置显式固定HiGHS 4线程、presolve和候选站进程并发策略；候选任务在独立进程中读取同一哈希RoadCase，按CPU和可用内存确定1～3个并发任务。新增TES投资敏感性SolveRequest，只允许同时缩放TES能量、功率和固定投资；输出区分求解器二进制、物理安装和实际充放状态。
- 目的：减少五候选站串行等待，同时避免把`built=1`但容量为0的求解器占位解释为TES已应用；任何敏感性结果均保留独立请求、有效RoadCase哈希、求解证据和QA。
- 验证：并行调度、序列化案例、请求哈希、失败传播和TES语义专项测试通过。真实全季敏感性`TES_SENSITIVITY_20260909_R2`中，冻结投资倍率1.0未采用TES；倍率0.5时实际容量4047.847 kWh_th、最大充/放热1921.526 kW_th，求解最优且独立QA通过。
- 边界：冻结经济主结果不因敏感性试验改变；既有R3全季仍是旧串行运行证据，新的全季并发总耗时需新RUN_ID重跑后才能声明。

# 2026-09-09 最终优化节点2：全季成果展示与评分证据

- 修改：新增合格全季结果读取、来源门禁和静态成果生成器，输出9组PNG/SVG、PDF图册、展示CSV/JSON、结果导读和CP评分追踪；TES图同时展示冻结零容量结论与0.5倍投资敏感性采用证据。新增`run.py report`及`tes-check`入口，不在绘图中重新求解或修改结果。
- 目的：把Pareto、成本/碳排分项、逐时调度、负荷热图、源—网—荷能流、GIS网络、TES及QA转为可直接用于比赛汇报的证据，同时明确研究范围和参数边界。
- 验证：报告来源、布尔数值、膝点、TES跨案例哈希、图表清单及失败门禁测试通过；`FINAL_20260909_R5/presentation`全部图实际渲染检查。展示工作簿含总览、Pareto点、代表方案、成本/碳排、独立QA和TES配对7张表，公式审计无错误。固定环境全量回归`744 passed / 3 skipped`。
- 边界：展示层只消费合格结果；图形不能证明求解合格，仍以ResultBundle、solver evidence和独立QA为准。

# 2026-09-09 最终优化节点3：白名单交付包与复现手册

- 修改：新增四包白名单工具，分别提取不可变Git源码、公开合成样例、权威膝点/TES采用方案证据和成果图册。加强qualified、optimal、1%认证gap、独立QA、TES静态容量及2160小时Parquet门禁；排除GUI、历史配置、旧输入适配器、原始数据、候选临时任务和工作簿预览。补充最终安装、运行、展示和打包手册。
- 目的：源码交付不夹带原始数据或大量历史文件，结果包不把失败运行、损坏逐时文件或仅有图片的材料误作合格证据，并允许TES结果目录迁移到另一台电脑后按情景ID重新定位。
- 验证：确定性ZIP、Git快照、脏工作树拒绝、路径越界、结果门禁、TES相对定位、损坏Parquet和图册输入边界专项测试通过；最终包仅在源码提交后从提交SHA生成。
- 边界：交付打包不会删除或修改仓库内容；历史命名的`legacy_economics.py`仍是现行经济适配器依赖，明确保留而不是误删。

# 2026-09-09 交付手册命令复核

- 修改：将白名单打包命令改为直接通过固定Conda前缀运行独立工具，并明确统一启动器只负责`run.py`子命令。
- 目的：避免把`tools/build_competition_release.py`误传给`run.py`而产生参数错误，确保新电脑可复制执行。
- 验证：固定环境路径与实际启动器分支逻辑逐行核对；最终交付包使用该命令实跑验证。

# 2026-09-09 Windows交付目录发布兼容

- 问题：四个ZIP和索引均已成功写入后，Windows在第一次目录改名时短暂返回`PermissionError`，手工退出进程后改名成功；内容校验和原始文件均未受影响。
- 修改：正式发布暂存目录时增加有限次数的垃圾回收和渐进短重试；目标目录已存在仍立即失败，不覆盖既有交付。
- 目的：兼容杀毒/索引程序对新ZIP的短时扫描锁，同时保持唯一暂存目录和失败证据不被删除。

# 2026-09-09 Windows交付目录发布收口

- 复验：短重试仍无法在同一Python进程退出前解除Windows对新ZIP目录的扫描锁，但进程结束后目录可立即改名，说明不是权限或内容错误。
- 修改：取消目录级原子改名，改为先创建唯一且禁止覆盖的目标目录，并用`_PACKAGE_BUILD_STATUS.json`记录`building/complete`状态；所有白名单内容预检仍在创建目录之前完成，失败目录保留用于诊断且不得原路径重试。
- 目的：避免依赖Windows对非空目录的即时改名，同时让部分写入与完整交付可以由机器状态明确区分。
