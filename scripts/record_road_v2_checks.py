...
def main():
    ...
    commands=[('environment',[sys.executable,'scripts/check_environment.py'],[0]),
              ('tests',[sys.executable,'-m','pytest','-q','--basetemp',str(pytest_root)],[0])]
    if args.osm_snapshot:
        commands.append(('v2_input_gate',[sys.executable,'scripts/run_case.py',
    ...
    evidence=dict(git_sha=subprocess.check_output(['git','rev-parse','HEAD'],cwd=repo,text=True).strip(),
        ...
        core_sha256=file_hash(repo/'competition/core_model.py'),commands=records,
