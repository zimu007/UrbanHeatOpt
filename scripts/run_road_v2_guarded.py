...
from competition.full_season_server import _available_memory_bytes
from competition.road_joint_v2.tasks import verify_plan, write_json
...
def run_guarded(root, task_id, requested_gib=None, max_wall_seconds=None):
    ...
    request=dict(command=command,cwd=str(Path(__file__).resolve().parents[1]),started_at=datetime.now(timezone.utc).isoformat(),
