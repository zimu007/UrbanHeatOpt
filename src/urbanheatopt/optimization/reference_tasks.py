...
from urbanheatopt.optimization.pareto import ParetoPoint, ParetoSpec, solve_pareto_task, assemble_pareto_run, select_representative_points
from urbanheatopt.optimization.solvers import SolverSettings, get_solver_evidence
from urbanheatopt.model.road_version import FROZEN_LEGACY_CORE_SHA256
...
from urbanheatopt.model.road_core import build_road_model, classify_direction_solution
...
def mathematics_hashes():
    result={str(p.relative_to(REPOSITORY)).replace('\\','/'):file_hash(p)
            for p in sorted((REPOSITORY/'src/urbanheatopt').rglob('*.py'))}
    if result['src/urbanheatopt/model/reference_core.py']!=FROZEN_LEGACY_CORE_SHA256:
...
def assemble(root, *, policy_carbon_cap=None):
    ...
    write_json(root/'representatives_provisional.json',dict(rules=representatives,certified_at_0_1_percent=False,
        note=(f"仅选择；尚未执行{plan['representative_gap']:.3%} gap复核求解，"
              "不宣称最终代表解认证完成")))
    from urbanheatopt.reporting.reference_figures import render_figures
    render_figures(root)
